...
def read_data(data_dir):
    """Read data and splits in two list: ham messages and spam messages"""
    spam = []
    ham = []
    with open(data_dir, 'r') as data_set:
        for line in data_set:
            ...
            if first_word == 'spam':
                spam.append(clean_text(rest))
            else:
                ham.append(clean_text(rest))
...
def main():
    ...
    (p_spam, p_dis_spam, p_ham, p_dis_ham, vocab) = learn_spam('dataset/small_dataset')
